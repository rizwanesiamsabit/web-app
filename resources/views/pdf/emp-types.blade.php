...
<head>
    <meta charset="utf-8">
    <title>Employee Types Report</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="title-box">
            <h1 class="report-title">Employee Types Report</h1>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 70%;">Employee Type Name</th>
        ...
        <tbody>
            @forelse($empTypes as $index => $empType)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $empType->name }}</td>
                    <td class="{{ $empType->status ? 'status-active' : 'status-inactive' }}">
    ...
    <div class="footer">
        <p>This report contains {{ $empTypes->count() }} employee type(s) | Generated by {{ $companySetting->company_name ?? 'System' }}</p>
    </div>
</body>
</html>